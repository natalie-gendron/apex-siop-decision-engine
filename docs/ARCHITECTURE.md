...
| **Outlook** | The simulated outcome distribution for a context — what the demand plan, pushed through supply and calibrated by evidence, is expected to produce; the **standing base outlook** = (base, ∅). In graphics: the solid center line and band. | The plan — the outlook moves with the context, the plan never does; P(plan) is the measured gap between them. |
